...
;(function (window) {
    ...
    iplayer.prototype = {
        ...
        // 全局变量；Object; 播放器对象
        V: null,
        // 全局变量；Array; 保存监听事件的数组
        listenerArr: [],
        // 全局变量；String;htmlvideo或者flashplayer
        playerType : '',
        /***********************************私有函数**********************************/
        // 参数初始化入口
        _init(config) {
        ...
        _getVideo() {
            ...
            if (this.html5Video) {
                ...
                if (this.config.videoDrawImage) {
                    ...
                    this.MDC = this.getDOMElement(cvid);
                    this.MDC_CONTEXT = canvas.getContext('2d');
                }
                // 设置当前的播放器类型为html5video
        ...
        _addEvent() {
            ...
            // 视频单击的事件
            this.addListener('click', eventVideoClick, this.V);
            this.addListener('click', eventVideoClick, this.MDC);

            // 视频播放的事件
            this.addListener('loadedmetadata', function () {
            ...
            // 视频缓冲事件
            this.addListener('waiting', function () {
                console.log('waiting')
            })
            // 视频seeked结束的事件
            this.addListener('seeked', function () {
                console.log('seeked')
            })
            // 视频播放结束的事件
            ...
            // 视频音量监听
            this.addListener('volumechange', function () {
        ...
        timer(time, fn, number){
            this.time = time || 10;              // 运行间隔
            this.fn = fn || null;                // 监听函数
            this.timeObj = null;                 // setInterval对象
            this.number = number || 0;           // 已经运行的次数
            this.numberTotal = null;             // 总需要的次数
            this.runing = false;                 // 当前的状态

            // 启动定时器的回调函数
            this.startFun = ()=> {
                this.number++;
                this.fn();
                if (this.numberTotal != null && this.number >= this.numberTotal) {
                    this.stop();
                }
            }
            ...
            this.start = ()=> {
                if (!this.runing) {
                    this.runing = true;
                    this.timeObj = window.setInterval(this.startFun, time);
                }
